...
const TimerControl: FC<TimerControlProps> = ({ end, driver, onFinish, onStart, onStop, onSkip, isBreak }) => {
  ...
  const maintimer = isNull(end) ? (
    <div className={classes.stop}>
      {isBreak ? (
        <>
          <LocalCafeIcon className={classes.driveIcon} style={{ fontSize: '3rem' }} />
        ...
        <>
          <DriveEtaIcon className={classes.driveIcon} style={{ fontSize: '3rem' }} />
          <Typography variant="h3" noWrap>
            {driver}
          </Typography>
          <Helmet>
